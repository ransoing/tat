// this is a barrel. we can use this to clean up imports in other files
// https://basarat.gitbooks.io/typescript/docs/tips/barrel.html
export * from './home/home.module';
export * from './home/home.page';
export * from './red-flags/red-flags.module';
export * from './red-flags/red-flags.page';
export * from './report/report.module';
export * from './report/report.page';
export * from './resources/resources.module';
export * from './resources/resources.page';
export * from './volunteer/volunteer.module';
export * from './volunteer/volunteer.page';
export * from './pre-outreach-survey/part-1/pos-part-1.page';
export * from './pre-outreach-survey/part-2/pos-part-2.page';
export * from './pre-outreach-survey/part-3/pos-part-3.page';
export * from './pre-outreach-survey/part-4/pos-part-4.page';
export * from './pre-outreach-survey/part-5/pos-part-5.page';
export * from './pre-outreach-survey/part-6/pos-part-6.page';
export * from './post-outreach-report/part-1/por-part-1.page';
export * from './post-outreach-report/part-2/por-part-2.page';
export * from './post-outreach-report/part-3/por-part-3.page';
export * from './post-outreach-report/part-4/por-part-4.page';
export * from './post-outreach-report/part-5/por-part-5.page';
